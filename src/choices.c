...
void choices_fread(choices_t* c, FILE* file, char input_delimiter) {
    ...
    while ((c->buffer_size += fread(c->buffer + c->buffer_size,
                                    1,
                                    capacity - c->buffer_size,
                                    file)) == capacity) {
        capacity *= 2;
        c->buffer = safe_realloc(c->buffer, capacity);
    }
...
static void worker_get_next_batch(struct search_job* job,
                                  size_t* start,
                                  size_t* end) {
...
static struct result_list merge2(struct result_list list1,
...
void choices_search(choices_t* c, const char* search) {
    ...
    for (int i = c->worker_count - 1; i >= 0; i--) {
        ...
        workers[i].result.list =
            malloc(c->size *
                   sizeof(struct scored_result)); /* FIXME: This is overkill */

        /* These must be created last-to-first to avoid a race condition when
         * fanning in */
        if ((errno = pthread_create(&workers[i].thread_id,
                                    NULL,
