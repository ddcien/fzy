...
static void clear(tty_interface_t* state) {
    ...
    size_t line = 0;
    while (line++ <
           state->options->num_lines + (state->options->show_info ? 1 : 0)) {
        tty_newline(tty);
    }
    tty_clearline(tty);
...
static void draw_match(tty_interface_t* state,
                       const char* choice,
                       int selected) {
    tty_t* tty = state->tty;
...
static void action_del_all(tty_interface_t* state) {
    memmove(state->search,
            &state->search[state->cursor],
...
static void action_pageup(tty_interface_t* state) {
    update_state(state);
    for (size_t i = 0;
         i < state->options->num_lines && state->choices->selection > 0;
         i++)
        choices_prev(state->choices);
}
...
void tty_interface_init(tty_interface_t* state,
                        tty_t* tty,
                        choices_t* choices,
                        options_t* options) {
    state->tty = tty;
...
static void handle_input(tty_interface_t* state,
                         const char* s,
    ...
    for (;;) {
        do {
            ...
            draw(state);
                                 ...
                                 state->ambiguous_key_pending ? KEYTIMEOUT : 0,
                                 0));
